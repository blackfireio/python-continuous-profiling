...
export PYTHON_PROBE_INTERNAL_VERSION=${INTERNAL_VERSION}
export PYTHON_PROBE_INTERNAL_REVCOUNT=${REV_COUNT}
export PYTHON_PROBE_VERSION=${VERSION}

update_version()
{
# add internal prefix&suffix if on master
if [[ ${PYTHON_PROBE_INTERNAL_VERSION} != "" ]]; then
    VERSION="${PYTHON_PROBE_INTERNAL_VERSION}"

    if [[ ${PYTHON_PROBE_INTERNAL_REVCOUNT} != "" ]]; then
        VERSION="${VERSION}.${PYTHON_PROBE_INTERNAL_REVCOUNT}"
    fi
...
# of binaries
echo -n ${VERSION} > ./src/sdk/VERSION
}
